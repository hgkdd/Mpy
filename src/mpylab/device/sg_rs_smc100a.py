...
#
#
# Für den Signalgnerator SMC100A mit dem Softwarepaket SMB-B106 zur Verwendung bis 6GHz wird die Klasse 'SMC100A' definiert.
...
class SIGNALGENERATOR(SGNLGNRTR):
    def __init__(self, **kw):
        ...
        self._cmds = {'Init': [('*RST', None),
                      ...
                      'SetFreq': [('SOUR:FREQ:CW {freq}Hz', None)],
                      ...
                      'GetLevel': [
                          (
                              'SOUR:POW:LEVEL:IMM:AMPL?',
                              lambda self, **kwargs: rf'(?P<level>{self._FP})'
                          )
                      ],
                      ...
                      'ConfAM': [
                          ...
                          ("SOUR:LFO:FREQ {freq} HZ", None),
                          ('SOUR:LFO:FREQ?', lambda self, **kwargs: rf'(?P<freq>{self._FP})'),
    ...
    def Init(self, ini=None, channel=None):
        ...
        #   
        # In der Methode 'main()' wird das Objekt sg der Klasse SMC100A definiert. Die Befehlsliste (dictionary) 'sg._cmds' der
        # Klasse SMC100A wird mit einem Eintag namens 'Preset' erweitert und bekommt als Wert zunächst eine leere Liste zugewiesen.
        ...
        presets = [
            ...
            #     None,
            #     (
            #         lambda self, v, from_u, v_conv, **kwargs:
            #             f"OUTP:ATT {self.convert.c2c(from_u, self._internal_unit, v_conv(v)):f} dB",
            #         None
            #     )
            # ),
            (
                ...
                (
                    lambda self, v, from_u, v_conv, **kwargs:
                    f"SOUR:POW:LEV:IMM:OFFS {self.convert.c2c(from_u, self._internal_unit, v_conv(v)):f}",
                    None
                )
            ),
            (
                ...
                (
                    lambda self, v, from_u, v_conv, **kwargs:
                    f"SOUR:POW:LIM:AMPL {self.convert.c2c(from_u, self._internal_unit, v_conv(v)):f}",
                    None
                )
            ),
            (
                ...
                (
                    lambda self, v, from_u, v_conv, **kwargs:
                    f"SOUR:POW:LEVEL:IMM:AMPL {self.convert.c2c(from_u, self._internal_unit, v_conv(v)):f}",
                    None
                )
            ),
            (
                ...
                if vals is None:
                    ...
                    if callable(cmd):
                        self._cmds['Preset'].append(
                            (
                                lambda _self, _cmd=cmd, _v=v, _from_u=from_u, _v_conv=v_conv, **kwargs:
                                _cmd(_self, v=_v, from_u=_from_u, v_conv=_v_conv, **kwargs),
                                tmpl
                            )
                        )
                    else:
...
def main():
    ...
    lv = quantities.Quantity(si.WATT, 0.001)
    print(lv)
    sg.SetLevel(lv)
...
#    ui.configure_traits()
    ...
    # # falls die Bedingung 'false' ist. Zuvor wird eine Testfrequenz und ein Level festgelegt, ein Objekt der Klasse SMC100A erzeugt und der
...
# das Programm direkt in die Funktion 'main()'. Bei der späteren Verwendung des Treibers wird nur die Klasse 'SMC100A' und deren
# Methoden importiert.
#
