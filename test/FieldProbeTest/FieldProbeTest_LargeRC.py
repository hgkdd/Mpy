    ...
    if option==1:
        ...
        try:
            ...
            for f in freq:
                ...
                #
                if 8e7<=f<=2e9:
                    p=scipy.linspace(scipy.sqrt(Pmin),scipy.sqrt(Pmax[0]),Pn[0])
                    power=p**2
                    #
                elif 2e9<f<=6e9:
                    p=scipy.linspace(scipy.sqrt(Pmin),scipy.sqrt(Pmax[1]),Pn[1])
                    power=p**2
                    #
                elif 6e9<f<=18e9:
                    p=scipy.linspace(scipy.sqrt(Pmin),scipy.sqrt(Pmax[2]),Pn[2])
                    power=p**2
                    #
                else:
                    continue
