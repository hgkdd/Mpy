...
##                       'M:\\umd-config\\largeMSC\\ini\\umd-narda-emc300-8-real.ini']
        ...
        for p in self.ports:
            self.eut[p]={}
            self.eut[p]['dev']=emc300.emc300(port=p-1)
            self.eut[p]['ok']=True
...
##            self.eut[_i].SetFreq(100e6)
    ...
    def test (self, stat):
        ...
        if cmd[0] == 'eut':
            ...
            self.messenger (umdutil.tstamp()+" Press %s to set user event"%str(self.keylist), [])
            dct={}
            while (time.time()-start < self.dwell):
                key = umdutil.anykeyevent()
                if key and chr(key) in self.keylist:
                    self.messenger(umdutil.tstamp()+" Got user event while EUT checking.", [])
...
##                            dct[_i] = 'stat=%s'%str(stat)                        
            ...
            if len(dct):
                self.messenger(umdutil.tstamp()+" EUT failure with: %r"%(dct), [])
...
##                            if stat == 0:
##                                notok.remove(_i)                        
                self.messenger(umdutil.tstamp()+" All EUTs OK.", [])                    
            else:
                self.messenger(umdutil.tstamp()+" All EUTs OK.", [])                
            ...
            fld = self.field()
            if fld:
                cmd = ('efield', '', {'efield': umddevice.UMDMResult(fld, umddevice.UMD_Voverm)})            
            else:
