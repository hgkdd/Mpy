...
class SMR(SIGNALGENERATOR):
    def __init__(self):
        ...
        self._cmds={'Init':     [('*RST', None),
                    ...
                    'SetLevel': [("':SOUR:POW:LEV:IMM:AMPL %f'%self.convert.scuq2c(unit, self._internal_unit, float(level))[0]", None)],
    ...
    def Init(self, ini=None, channel=None):
        ...
        presets=[('attmode',
                 ...
                 ('leveloffset',
                      None,
                      ("':SOUR:POW:LEV:IMM:AMPL:OFFS %f'%self.convert.c2c(self.levelunit, self._internal_unit, float(v))", None)),
                 ('levellimit',
                      None,
                      ("':SOUR:POW:LIM:AMPL %f'%self.convert.c2c(self.levelunit, self._internal_unit, float(v))", None)),
                 ('level',
                      None,
                      ("':SOUR:POW:LEV:IMM:AMPL %f'%self.convert.c2c(self.levelunit, self._internal_unit, float(v))", None)),
                 ('outputstate',
                      [('1','on')],
