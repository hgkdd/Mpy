    ...
    def _GetNominal_wrap(self, method):
        if isinstance(method, ct._CFuncPtr):
            def m(state, what):
                ...
                c_instance=ct.c_int(self.instance)
                method.restype=ct.c_int
                retval = method(ct.byref(c_nominal_r), ct.byref(c_nominal_i), c_what, c_state, c_instance, ct.byref(c_error))
                self.error=c_error.value
                if not self.error and retval==0:
    ...
    def _GetTrace_wrap(self, method):
        if isinstance(method, ct._CFuncPtr):
            def m(freq, what, rows, points, size):  # TODO
                ...
                method.restype=ct.c_int
                retval = method(ct.byref(c_data), c_freq, c_what, c_rows, c_points, c_size, c_instance, ct.byref(c_error))
                self.error=c_error.value
##                if not self.error:
##                    obj=self.cdata_to_obj(c_data)
##  
    ...
    def _GetTraceNB_wrap(self, method):
        if isinstance(method, ct._CFuncPtr):
            def m(freq, what, rows, points, size, ret_stat):  # TODO
                ...
                method.restype=ct.c_int
                retval = method(ct.byref(c_data), c_freq, c_what, c_rows, c_points, c_size, c_ret_stat, c_instance, ct.byref(c_error))
                self.error=c_error.value
##                if not self.error:
##                    obj=self.cdata_to_obj(c_data)
##  
...
class CONVERT(object):
    ...
    def scuq2c (self, Sunit, Cunit, data):
        ...
        for item in data:
            if uconf[1]!=None:  # Unit => dB (no dB in scuq available)
